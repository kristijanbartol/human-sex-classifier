...
def project(kpts_3d, proj_mat):
    ...
    '''
    ones_vector = torch.ones([1, kpts_3d.shape[1]], dtype=torch.float32)
    kpts_3d = torch.cat((kpts_3d, ones_vector))
    kpts_2d = torch.mm(proj_mat, kpts_3d)
    #last_row = kpts_2d[2, :].reshape((1, kpts_2d.shape[1]))
    #kpts_2d_hom = torch.mul(kpts_2d, 1. / last_row)
    return kpts_2d.transpose(0, 1)
    '''
